...
import { Building2, ArrowRight, MapPin, CheckCircle, Bug } from "lucide-react";
...
export default function Onboarding() {
  ...
  const fetchDebugInfo = useCallback(async () => {
    ...
    let hotelCount = 0;
    if (orgId) {
      const { data: hotels } = await supabase.from("hotels").select("id").eq("organization_id", orgId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const { data: orgId, error: orgError } = await supabase.rpc("create_org_and_owner", { _org_name: orgName.trim() });
      if (orgError) throw orgError;

      const { data: existingHotels } = await supabase.from("hotels").select("id").eq("organization_id", orgId);
      ...
      const { data: hotelId, error: hotelError } = await supabase.rpc("create_hotel_for_org", {
        _org_id: orgId, _name: hotelName.trim(), _city: selectedCity.name,
        _rooms: parseInt(rooms) || 85, _base_price: parseFloat(basePrice) || 120,
      });
      if (hotelError) {
      ...
      await supabase.from("hotels").update({
        city_name: selectedCity.name, country_code: selectedCity.country,
        latitude: selectedCity.lat, longitude: selectedCity.lon,
      }).eq("id", hotelId);

      await supabase.rpc("set_active_hotel", { _hotel_id: hotelId });
      await refreshMemberships();
      toast({ title: "Welcome to QonaqAI!", description: `${orgName} and ${hotelName} created.` });
      navigate("/dashboard", { replace: true });
    } catch (err: any) {
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="w-full max-w-md space-y-6">
        <div className="text-center">
          ...
          <h1 className="mt-6 text-2xl font-display font-bold">Set up your hotel</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="rounded-xl border border-border bg-card p-5 space-y-4">
            <div className="flex items-center gap-2 text-sm font-medium">
              <Building2 className="h-4 w-4 text-primary" /> First Hotel
            </div>
            <div className="space-y-2">
              <Label htmlFor="hotelName">Hotel Name</Label>
              <Input id="hotelName" value={hotelName} onChange={(e) => setHotelName(e.target.value)} placeholder="Grand Hotel" required />
            </div>
            <div className="space-y-2 relative" ref={suggestionsRef}>
              <Label htmlFor="city">City</Label>
              <div className="relative">
                <Input id="city" value={cityQuery} onChange={(e) => handleCityInput(e.target.value)} onFocus={() => citySuggestions.length > 0 && setShowSuggestions(true)} placeholder="Search city…" required autoComplete="off" />
                {loadingCities && <div className="absolute right-2 top-1/2 -translate-y-1/2"><div className="h-4 w-4 animate-spin rounded-full border-2 border-primary border-t-transparent" /></div>}
              </div>
              {showSuggestions && citySuggestions.length > 0 && (
                <div className="absolute z-50 top-full mt-1 w-full rounded-lg border border-border bg-card shadow-lg overflow-hidden">
                  {citySuggestions.map((s, i) => (
                    <button key={`${s.name}-${s.country}-${i}`} type="button" onClick={() => selectCity(s)} className="flex w-full items-center gap-2 px-3 py-2.5 text-left text-sm hover:bg-accent transition-colors" title={s.label}>
          ...
          <Button type="submit" className="w-full" disabled={submitting || !selectedCity}>
            {submitting ? "Setting up…" : "Launch QonaqAI"} <ArrowRight className="ml-2 h-4 w-4" />
          </Button>
        </form>
      </div>
    </div>
